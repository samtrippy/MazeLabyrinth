...
class BombsMazeFactory(MazeFactory):
    @classmethod
    def MakeWall(cls):
        return BombedWall()
    ...
    @classmethod
    def MakeRoom(cls, n):
        return RoomWithABomb(n) #pass in the roomNo, create a spell
    
class BombedWall(Wall):
    def __init__(self):
        self.wall_is_damaged = False #True if wall has been bombed / bomb has exploded
...
#    play_game(MazeFactory) # pass in the class directly
#    play_game(MazeFactory()) #pass in an instance of the class
    play_game(BombsMazeFactory)
    play_game(BombsMazeFactory, explode_bomb=True)